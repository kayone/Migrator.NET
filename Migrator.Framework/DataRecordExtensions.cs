...
namespace Migrator.Framework
{
    public static class DataRecordExtensions
    {
        ...
        public static T TryParse<T>(this IDataRecord record, string name, Func<T> defaultValue)
        {
            ...
            {
                if (value is Int32 || value is Int64 || value is Int16 || value is UInt16 || value is UInt32 ||
                    value is UInt64)
                {
            ...
            catch (InvalidCastException ex)
            {
                throw new MigrationException(
                    string.Format("Invalid cast exception of value: {0} of type: {1} to type: {2} (field name: {3})",
                                  value, value.GetType(), typeof (T), name), ex);
            }
        }
    }
}